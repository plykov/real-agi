...
## 0. TL;DR — seven maps turned out to be one map <a id="tldr"></a>
...
## 1. From verticals to the axes of a single space <a id="1-axes"></a>
...
flowchart LR
    ...
    subgraph SC["System coordinates"]
        direction TB
        S1["Time"]
        S2["Liquidity"]
        S3["Load"]
    ...
    DA --> G(["Coordinate grid:<br/>the person's position,<br/>specified along all axes at once"])
    SC --> G
```
...
## 2. The person as a point: drift instead of an answer <a id="2-drift"></a>
...
## 3. Everything is a signal: behavior instead of self-report <a id="3-signal"></a>
...
## 4. Why accuracy grows with the number of users <a id="4-scale"></a>
...
## 5. The personal analytical center: functions and contracts <a id="5-center"></a>
...
```mermaid
flowchart TB
    subgraph PAC["Personal analytical center"]
        F1["Observation<br/>& mapping"]
        F2["Scenarios<br/>& trajectories"]
        F3["Positioning<br/>& portfolio"]
        F4["Interpretation &<br/>communication"]
        F5["Safety &<br/>constraints"]
    end
    P(("The person:<br/>a point in the multidimensional<br/>space of life"))
    PAC --> P
```
...
## 6. The Guardian: a form, not a fairy <a id="6-form"></a>

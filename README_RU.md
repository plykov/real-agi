...
> **🧭 Дорожная карта:** взгляд вперёд — как ядро, однажды инкапсулированное, распространяется как донор по вертикалям и завершается системой самообучения — в [`ROADMAP_RU.md`](ROADMAP_RU.md). (Раздел `3_Verticals/` — та же архитектура, прочитанная *задом наперёд*, как индуктивное доказательство.)
...
├── strategy/             ← стратегические заметки (рус.): куда инвестировать внимание в эпоху агентов
├── ROADMAP_RU.md         ← взгляд вперёд: концепция → ядро-донор → вертикали → самообучение
└── README.md
```
...
- **[`mentoring-engine-architecture.md`](Ru/2_tech%20level/mentoring-engine-architecture.md)** — *как*: двухуровневая система «самообучающееся ядро + смертные вертикали»; что можно дистиллировать, а что нет (запас против потока); фитнес-функция «победа = процветание клиента». *(Часть 3 трилогии о менторинге.)*
...
Небольшой отдельный слой верхнего уровня: стратегические меморандумы о том, **куда инвестировать внимание и время в эпоху агентов** — когда сами агенты и инженерия вокруг них коммодитизируются. Это слой мышления над корпусом, а не документация по инструментам. *(Не путать с папкой-проектом `Ru/3_Verticals/strategy/` — то же слово, другое содержание.) Импортировано из репозитория автора [`essays`](https://github.com/AI-agents-incubator/essays).*

- **[`Where to Invest Your Time in the Age of Agents.md`](strategy/Where%20to%20Invest%20Your%20Time%20in%20the%20Age%20of%20Agents.md)** — основной меморандум: почему агенты становятся коммодити и почему ключевой актив смещается к суждению, вкусу и новым формам опыта («фабрика дофамина» как эволюционный конвейер контента).
...
См. [`strategy/README.md`](strategy/README.md) — путеводитель по папке и порядок чтения.
...
- Различай статус (approved/draft); весь корпус опубликован в публичном регистре.
...
- Документы несут YAML-frontmatter со статусом (`draft` / `approved`) и регистром. Весь корпус опубликован в **публичном** регистре.

...
│   ├── 1_Concept/        Серия «Траектории» — концептуальное ядро (5 эссе + 2 варианта)
...
1. **Концепция:** `Ru/1_Concept/` — эссе 1 → 2 → 3 → 4 → 5 (редакции `.variant-game-drama` опциональны: та же идея, другой словарь).
...
ЧИТАЙ файлы в этом порядке (пропусти *.variant-game-drama.md — это альтернативные
редакции; англоязычные версии каждого документа лежат в Eng/ по идентичным путям):
